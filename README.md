...
Gulls is a small, zero-dependency framework for minimal WebGPU setup, targeting
...
## Use
gulls.js is designed to be used without any type of build stage required. 
Import the main `gulls.js` file and any helpers you might want to 
...
and it's important that you use the same indices in your shader that gulls
uses on the CPU. gulls uses the order of the `data` array to determine these binding
indices.
...
Almost all functionality requires first making an instance of gulls using a call
to `gulls.init()`. From there, methods consist of one of three types:
...
### Data
- `sg.uniform( number | array )`. This method returns a gulls uniform object that
...
pipeline. It expects a JS object with the following properties:
  ...
  - `data` (optional): An array of gulls `data` objects, as described in the previous section. While this is technically optional, almost all render shaders will use this property in some way.
...
a shader pipeline. It expects a JS object with the following properites:
  - `shader`: The text of the render shader, including both the vertex and fragment shader.
  - `data` (optional): An array of gulls `data` objects, as described in the previous section.
...
- [The Schwartz](https://charlieroberts.codeberg.page/TheSchwartz) : A browser-based WGSL live coding environment made using gulls.js
